...
with open('build.sh', 'w') as f:
    ...
    for c_no, container in enumerate(lst):
        f.write(f"sudo singularity build {container} docker://quay.io/biocontainers/{container} > /dev/null 2>&1 && rsync -azqe 'ssh StrictHostKeyChecking=no' ./{container} singularity@depot.galaxyproject.org:/srv/nginx/depot.galaxyproject.org/root/singularity/ && rm {container} && echo 'Container {c_no} ({container}) of {len(lst)} built.'\n")
